                        ...
                        <div class="ps-product__info col-md-6">
                            @if( \EasyShop\Models\Product::hasDiscount( $product->discount ) )
                                <h3 class="ps-product__price">
                                    {{trans('globalstore.price')}}: {{(int)(EasyShop\Models\Product::getPriceRetail1($product, false, 0))}} {{\EasyShop\Models\AdminSettings::getField('currency')}}
                                    <del>
                                        {{(int)$product->$myPriceGroup}}  {{\EasyShop\Models\AdminSettings::getField('currency')}}
                                    </del>
                                    <p class="lapTopTop saveCustom save-ammount">
                                        {{trans('globalstore.youSave')}} : {{ (int) ($product->$myPriceGroup - EasyShop\Models\Product::getPriceRetail1($product, false, 0) ) . " " . \EasyShop\Models\AdminSettings::getField('currency') }} !
                                    </p>
                                </h3>
                                <i style="color:#d24427 !important; font-style:normal; font-size:24px !important;"> {{trans('globalstore.risk')}}</i>
                            ...
                            <p>
                                {{trans('globalstore.code')}}: {{$product->unit_code}}
                            </p>
                                <div class="garancija-cont">
                                    <img src="/assets/global-store/images/crvenbaner.png" alt=""/>
                                </div>
                            <div class="ps-product__block ps-product__size">

                                <div class="form-group">
                                    <h4 style="float:left; margin-top:12px; padding-right: 10px;" class="mb-10">{{trans('globalstore.quantity')}}: </h4><input style="width:80px;" class="form-control" data-product-quantity="" type="number" value="1">
                                </div>
                            ...
                            <div class="ps-product__shopping"><a style="" class="customButton ps-btn mb-10" value="{{$product->id}}" data-add-to-cart="" href="#">{{trans('globalstore.buy')}}<i class="ps-icon-next"></i></a>
                            </div>
                                ...
                                <div class="ps-product__content mt-15">
                                    <ul class="tab-list" role="tablist">
                                        <li class="active"><a aria-controls="tab_01" role="tab" data-toggle="tab">{{trans('globalstore.description')}}</a></li>
                                    </ul>
                                </div>
                ...
                <div class="ps-section__header mb-50">
                    <div class="row">
                        <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 ">
                            <h3 class="ps-section__title" data-mask="{{trans('globalstore.similar')}}">- {{trans('globalstore.similarProducts')}}</h3>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="related-products-slider">
                            @foreach($relatedProducts as $article)
                                ...
                                <div class="single-product-grid col-md-3">
                                    ...
                                    {{--Додади во кошничка--}}
                                    {{--</a>--}}
                                    <a class="ps-btn mt-10" value="{{$article->id}}" data-add-to-cart="" href="{{route('product', [$article->id, $article->url])}}">{{trans('globalstore.buy')}} <i class="ps-icon-next"></i></a>
                                </div>
